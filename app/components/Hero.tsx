...
const CODE_SNIPPETS = [
  { code: "const build = await compile();", color: "var(--accent-2)", delay: 0 },
  { code: "await db.migrate();", color: "var(--accent)", delay: 1.5 },
  { code: "return response.json();", color: "var(--accent-3)", delay: 3 },
  { code: "deploy();", color: "var(--accent-4)", delay: 4.5 },
];
  ...
  return (
    <section ref={sectionRef} id="home"
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{ background:"radial-gradient(ellipse 80% 80% at 50% 48%, transparent 18%, rgba(var(--shadow-rgb),.84) 100%)" }}
        aria-hidden="true" />
      ...
      {[
        ...
        <motion.div key={i} className={`absolute ${cls} pointer-events-none`}
          ...
          <svg width="32" height="32" viewBox="0 0 32 32" style={{ transform:`rotate(${rot}deg)` }}>
            <path d="M2 30 L2 2 L30 2" stroke="rgba(var(--accent-2-rgb),0.45)" strokeWidth="1.5" fill="none"/>
          </svg>
        </motion.div>
      ))}
      ...
      <motion.div className="absolute left-0 right-0 h-px pointer-events-none z-[3]"
        style={{ background:"linear-gradient(90deg,transparent,rgba(var(--accent-2-rgb),.34),rgba(var(--accent-3-rgb),.28),transparent)" }}
        animate={{ top:["-2px","102%"] }}
        ...
        <motion.div variants={stagger} initial="hidden" animate="show"
          ...
          <motion.div variants={up} className="flex justify-center lg:justify-start mb-7">
            <motion.span
              ...
              animate={{ boxShadow:["0 0 0 rgba(var(--accent-3-rgb),0)","0 0 24px rgba(var(--accent-3-rgb),.35)","0 0 0 rgba(var(--accent-3-rgb),0)"] }}
          ...
          <motion.h1 variants={up}
            ...
            <span className="inline-block lg:block lg:mr-0 mr-4">
              {FIRST.map((ch, i) => (
                <motion.span key={i} className="inline-block"
                  style={{ color:"var(--fg)", textShadow:"0 0 30px rgba(var(--accent-rgb),.22), 0 2px 0 rgba(0,0,0,.5)" }}
            ...
            <span className="inline-block lg:block">
              {LAST.map((ch, i) => (
                <motion.span key={i} className="inline-block"
                  style={{ color:"var(--accent)", textShadow:"0 0 26px rgba(var(--accent-3-rgb),.28)" }}
                  initial={{ opacity:0, rotateX:-90, y:30 }}
                  animate={{ opacity:1, rotateX:0, y:0 }}
          ...
          <motion.p variants={up} className="text-xl sm:text-2xl font-light mb-2"
            style={{ color:"rgba(var(--fg-rgb),0.82)" }}>
            ...
            {titleParts[1] ? (
              <>
                ...
                <motion.span className="font-bold"
                  style={{ color:"var(--fg)" }}
                  animate={{ textShadow:["0 0 8px rgba(var(--accent-3-rgb),.35)","0 0 28px rgba(var(--accent-rgb),.7)","0 0 8px rgba(var(--accent-3-rgb),.35)"] }}
          ...
          <motion.div variants={up}
            ...
            style={{ color:"rgba(var(--muted-rgb),0.7)" }}>
            <MapPin size={11} />
          ...
          <motion.p variants={up}
            ...
            style={{ color:"rgba(var(--fg-rgb),0.72)" }}>
            {site.tagline}
          </motion.p>
          ...
          <motion.div variants={up} className="flex justify-center lg:justify-start gap-3">
            {[
              { href:site.github,              icon:<GithubIcon width={18} height={18} />,   label:"GitHub",   color:"rgba(var(--accent-2-rgb),0.9)" },
              { href:site.linkedin,            icon:<LinkedinIcon width={18} height={18} />, label:"LinkedIn", color:"rgba(var(--accent-rgb),0.9)" },
              { href:`mailto:${site.email}`,   icon:<Mail size={18} />,                      label:"Email",    color:"rgba(var(--accent-3-rgb),0.9)" },
              { href:whatsAppChatUrl(site.phone), icon:<WhatsAppIcon size={18} />,           label:"WhatsApp", color:"rgba(var(--accent-rgb),0.9)" },
              ...
              <motion.a key={label} href={href}
                ...
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = "rgba(var(--muted-rgb),0.7)"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(var(--accent-2-rgb),0.16)"; (e.currentTarget as HTMLElement).style.boxShadow = "none"; }}>
                {icon}
              </motion.a>
            ))}
          </motion.div>
      ...
      <motion.button
        ...
        style={{ color:"rgba(var(--muted-rgb),0.62)" }}
  ...
  return (
    <div className="relative mx-auto max-w-[520px]">
      <motion.div className="absolute -inset-6 rounded-[2rem] opacity-70"
        style={{ background:"radial-gradient(circle at 50% 35%,rgba(var(--accent-rgb),.22),transparent 62%)", filter:"blur(20px)" }}
        animate={{ scale:[1,1.05,1], opacity:[.5,.85,.5] }}
        transition={{ duration:5, repeat:Infinity }} />

      <div className="relative overflow-hidden rounded-[1.6rem] p-5"
        style={{
          background:"linear-gradient(145deg,rgba(var(--card-from-rgb),.9),rgba(var(--panel-deep-rgb),.86))",
          border:"1px solid rgba(var(--accent-2-rgb),.17)",
          boxShadow:"0 28px 90px rgba(var(--shadow-rgb),.46), inset 0 1px 0 rgba(var(--accent-2-rgb),.08)",
        }}>
        <div className="mb-5 flex items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[.24em]" style={{ color:"rgba(var(--accent-2-rgb),.72)" }}>
              Backend Workflow
            </p>
        ...
        <div className="rounded-[1.2rem] p-4"
          style={{ background:"rgba(var(--panel-deep-rgb),.66)", border:"1px solid rgba(var(--accent-2-rgb),.1)" }}>
          <div className="space-y-3">
            {[
              { w:"78%", color:"var(--accent-2)" },
              { w:"58%", color:"var(--accent)" },
              { w:"86%", color:"var(--accent-3)" },
              { w:"44%", color:"var(--accent-4)" },
            ].map((line, i) => (
              <div key={i} className="flex items-center gap-3">
                <span className="w-5 text-right text-[10px] font-mono" style={{ color:"rgba(var(--muted-rgb),.45)" }}>
          ...
          <motion.div className="mt-5 h-10 rounded-2xl flex items-center px-4 font-mono text-xs"
            style={{ background:"rgba(var(--tint-rgb),.34)", color:"var(--accent-2)", border:"1px solid rgba(var(--accent-2-rgb),.1)" }}
            animate={{ boxShadow:["0 0 0 rgba(var(--accent-rgb),0)","0 0 24px rgba(var(--accent-rgb),.16)","0 0 0 rgba(var(--accent-rgb),0)"] }}
            transition={{ duration:2.6, repeat:Infinity }}>
            <span>php artisan route:list</span>
            <motion.span className="ml-1 inline-block h-4 w-1 rounded-sm bg-[var(--accent-2)]"
          ...
          {workflow.map((item) => (
            <motion.div key={item.label}
              ...
              <div className="mb-2 inline-flex h-8 w-8 items-center justify-center rounded-xl"
                style={{ color:"var(--accent-2)", background:"rgba(var(--accent-rgb),.13)" }}>
                {item.icon}
              </div>
              <p className="text-xs font-bold" style={{ color:"var(--fg)" }}>{item.label}</p>
              <p className="mt-1 text-[10px] leading-snug" style={{ color:"rgba(var(--fg-rgb),.56)" }}>{item.value}</p>
            </motion.div>
          ))}
        </div>
      </div>
  ...
  return (
    <motion.a href={href} onClick={handleClick}
      ...
      className={`relative px-8 py-3.5 rounded-full text-sm font-semibold overflow-hidden transition-all duration-300 ${
        primary ? "text-[var(--ink)]" : "text-[var(--accent-2)]"
      }`}
      style={primary ? {
        background:"var(--btn-bg)",
        boxShadow:"0 18px 42px rgba(var(--accent-3-rgb),.3)",
      } : {
        background:"rgba(var(--panel-rgb),0.6)",
        border:"1px solid rgba(var(--accent-2-rgb),0.28)",
        boxShadow:"0 0 0 rgba(var(--accent-3-rgb),0)",
      }}>
      {primary && (
